...
CREATE TABLE IF NOT EXISTS key_metrics_base (
    ...
    booking_charge_aed_per_day DOUBLE AS (
        CASE
            WHEN TIMESTAMPDIFF(MINUTE, pickup_datetime, return_datetime) / (24 * 60) > 0 THEN booking_charge_aed / (TIMESTAMPDIFF(MINUTE, pickup_datetime, return_datetime) / (24 * 60))
    ...
    booking_charge_less_discount_aed_per_day DOUBLE AS (
        CASE
            WHEN TIMESTAMPDIFF(MINUTE, pickup_datetime, return_datetime) / (24 * 60) > 0 THEN booking_charge_less_discount_aed / (TIMESTAMPDIFF(MINUTE, pickup_datetime, return_datetime) / (24 * 60))
    ...
    return_hours_to_midnight INT AS (HOUR(TIMEDIFF('24:00:00', return_time))) VIRTUAL,
    return_minutes_to_midnight INT AS (MINUTE(TIMEDIFF('24:00:00', return_time))) VIRTUAL,
...
INSERT INTO key_metrics_base (booking_id, status, booking_type, vendor, is_repeat, country, booking_charge_aed, booking_charge_less_discount_aed, pickup_date, pickup_datetime, return_date, return_datetime)
SELECT booking_id, status, booking_type, marketplace_or_dispatch AS vendor, repeated_user AS is_repeat, deliver_country AS country, booking_charge_aed, booking_charge_less_discount_aed, pickup_date, pickup_datetime, return_date, return_datetime
