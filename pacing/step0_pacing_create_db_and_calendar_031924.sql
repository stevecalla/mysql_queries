...
DROP DATABASE IF EXISTS ezhire_pacing_metrics;

-- CREATE RENTAL RECORD TABLE
CREATE DATABASE ezhire_pacing_metrics;
...
-- Insert data for the years 2023 and the last day of the current year
...
FROM (
    SELECT
        DATE_ADD('2022-01-01', INTERVAL seq DAY) AS date_seq
    FROM (
        SELECT
    ...
    WHERE DATE_ADD('2022-01-01', INTERVAL seq DAY) BETWEEN '2022-01-01' AND DATE_ADD(LAST_DAY(DATE_ADD(NOW(), INTERVAL 12-MONTH(NOW()) MONTH)), INTERVAL 1 YEAR) -- two years from now
) AS calendar_data;
