...
SELECT * FROM information_schema.TABLES WHERE TABLE_SCHEMA = 'myproject' AND TABLE_NAME = 'auth_user';
SHOW COLUMNS FROM rental_fuser;
...
SELECT
    ...
    CASE
        ...
        -- WHEN auth_user.first_name LIKE '%בסד הנדסה%' THEN REPLACE(auth_user.first_name, '%\\%', '0')
        -- WHEN auth_user.first_name LIKE '%בסד הנדסה%' THEN SUBSTRING(auth_user.first_name, 1, LENGTH(auth_user.first_name) - 2)
        -- WHEN auth_user.first_name LIKE '%\\%' THEN SUBSTRING(auth_user.first_name, 1, LENGTH(auth_user.first_name) - 2)
        -- WHEN auth_user.first_name LIKE '%\\%' THEN 'zzzz'

        WHEN auth_user.first_name LIKE "%Fx %" THEN ''
        -- WHEN auth_user.first_name LIKE "%Fx %" THEN REPLACE(auth_user.first_name, '\\', '')
